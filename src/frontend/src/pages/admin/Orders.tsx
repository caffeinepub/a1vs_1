...
export default function Orders() {
  ...
  const { data: orders = [], isLoading } = useQuery<Order[]>({
    ...
    queryFn: async () => {
      const raw = await actor!.getAllOrders(token);
  ...
  const assignInvoiceMutation = useMutation({
    ...
    onError: (err: unknown) => {
      const msg =
        err instanceof Error ? err.message : "Failed to assign invoice number";
      toast.error(msg);
    },
  });
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    {order.status === "delivered" && !order.invoiceNumber && (
                      <Button
                        ...
                        onClick={() =>
                          assignInvoiceMutation.mutate(order.orderId)
                        }
                      >
          ...
          <div className="space-y-4 py-2">
            <p className="text-sm text-muted-foreground">
              ...
              <span className="font-semibold text-foreground">
                {deletingOrder?.invoiceNumber
                  ? `INV# ${deletingOrder.invoiceNumber}`
        ...
        >
          ...
          <DialogFooter className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            ...
            <Button
              ...
              className="gap-2"
              disabled={isAssigning}
              data-ocid="admin.assign_rider.no_rider_button"
              onClick={async () => {
